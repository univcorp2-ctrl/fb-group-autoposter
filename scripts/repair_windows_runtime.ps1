...
if (Test-Path $GitDir) {
  Get-ChildItem -Path $GitDir -Filter 'desktop.ini' -Recurse -Force -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
}
...
function Register-FBTask {
  param([string]$Name,[string]$Script,[string]$At,[int]$RandomDelayMin=0,[int]$ExecutionHours=1,[switch]$AtLogon)
  $args = "-NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$LauncherPath`" -Script `"$Script`""
...
Register-FBTask 'FBAutoposter-Morning' 'scripts\run_daily_drive.py' '09:30' 45 -ExecutionHours 8 -AtLogon
Register-FBTask 'FBAutoposter-Midday' 'scripts\run_daily_drive.py' '13:00' 30 -ExecutionHours 8
Register-FBTask 'FBAutoposter-Afternoon' 'scripts\run_daily_drive.py' '16:30' 30 -ExecutionHours 8
Register-FBTask 'FBAutoposter-Evening' 'scripts\run_daily_drive.py' '20:30' 45 -ExecutionHours 8
