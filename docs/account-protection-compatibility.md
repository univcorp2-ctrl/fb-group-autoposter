...
| Backup/restore | `backup_profile`, `restore_profile`; `test_current_checkpoint_flow_restores_once_then_retries_before_stopping`; `tests/test_session_restore.py` | keep 7 backups; current ensure flow permits at most one restore for both ordinary expiry and `CheckpointRequired` | Task 4 clone-only recovery | Backup excludes volatile locks and recovery is bounded to one restore; the planned Task 4 change will restrict candidate restore to ordinary expiry and prohibit challenge restore/retry. |
