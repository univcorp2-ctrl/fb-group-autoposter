"""Resolve downloaded EstateBoard images from the Google Drive property archive.

The archive is intentionally treated as read-only.  Matching is performed using
property id first and a normalized property title second.  Results are cached for
the lifetime of one posting process so a large Drive tree is not scanned once per
group.
"""
...
def _folder_index(root_text: str) -> tuple[tuple[str, str, str], ...]:
    """Return tuples of (normalized folder name, raw folder name, path)."""
    root = Path(root_text)
    if not root.exists():
        return ()
...
def resolve_property_images(
    ...
    *,
    ...
    """Find local image files belonging to an EstateBoard item.

    Returns an empty list when the Drive mount is unavailable.  This keeps the
    text-only posting path operational instead of turning a Drive outage into a
    complete posting outage.
    """
    if not drive_root:
        return []
...
def attach_drive_images(
    ...
    """Attach Drive images to mapped property dictionaries in-place."""
